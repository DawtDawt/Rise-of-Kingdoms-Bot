...
class SelectedDeviceFrame(Frame):
    ...
    def config_frame(self):
        ...
        for i in range(len(atf.bot_config_title_fns)):
            ...
            check = section_frame(self, inner_frame, title_fns, sub_fns)
            check.grid(row=i, column=0, sticky=N + W)

        inner_frame.update_idletasks()

        frame_canvas.config(
            width=self.windows_size[0] - 20, height=self.windows_size[1] - 350
        )
        canvas.config(
            width=self.windows_size[0] - 20,
            height=self.windows_size[1] - 350,
    ...
    def start(self):
        ...
        self.bot.building_pos_update_event = lambda **kw: write_building_pos(
            kw["building_pos"], kw["prefix"]
        )
        self.bot.config_update_event = lambda **kw: write_bot_config(
            kw["config"], kw["prefix"]
        )
    ...
    def bottom_frame(self):
        ...
        building_pos_button.grid(row=0, column=1, padx=(0, 5), sticky=N + W)

        # dev tool position setting
        def on_dev_tool(btn):
            if self.dev_tool_window is None:
                self.dev_tool_window = dev_tool_window(self)

        building_pos_button = button(frame, on_dev_tool, text="Dev Tool")
...
def building_pos_window(parent):
        ...
        def on_set_click(btn):
            if selected_building["name"] is not None:
                selected_building["label"].config(
                    text="[{}, {}]".format(
                        selected_building["prev_pos"][0],
                        selected_building["prev_pos"][1],
                    )
                )
    ...
    def image_frame(parent):
        ...
        label = Label(
            frame, text="Loading city image, Please wait!", background="white"
        )
    ...
    def right_frame(parent):
        ...
        y_scrollbar = Scrollbar(
            frame_right, orient="vertical", command=canvas_right.yview
        )
        y_scrollbar.grid(row=0, column=1, sticky="ns")
    ...
    parent.building_pos_window.title(
        "{} Building Position".format(parent.device.serial.replace(":", "_"))
    )
...
def dev_tool_window(parent):
    ...
    def building_name_xy_config_frame(master, row, name, pos):
        ...
        pos_label.grid(row=row, column=1, pady=2, sticky=W)
        set_button.grid(row=row, column=2, pady=2, sticky=NSEW)
        retake_button.grid(row=row, column=3, pady=2, sticky=NSEW)

    def close_window():
        parent.dev_tool_window.grab_release()
        parent.dev_tool_window.destroy()
        parent.dev_tool_window = None

    def image_frame(parent):
        frame = Frame(parent.dev_tool_window, width=640, height=360)
        frame.grid_rowconfigure(0, weight=1)
        frame.grid_columnconfigure(0, weight=1)
        frame.grid_propagate(False)
        frame.grid(row=0, column=0, sticky=N + W)
        ...
        def after_image_load():
            image = parent.bot.get_current_image().resize((640, 360), Image.ANTIALIAS)
            frame.image = image = ImageTk.PhotoImage(image)
            canvas.create_image((0, 0), image=image, anchor="nw")
            parent.bot.curr_thread = None
            label.grid_forget()
    ...
    def set_focus(event):
        parent.dev_tool_window.attributes("-topmost", 1)
        parent.dev_tool_window.attributes("-topmost", 0)
        parent.dev_tool_window.focus_force()

    parent.dev_tool_window = Toplevel(parent.master.master.master)
    parent.dev_tool_window.resizable(0, 0)
    parent.dev_tool_window.grab_set()

    parent.dev_tool_window.title(
        "{} Dev Tool".format(parent.device.serial.replace(":", "_"))
    )
    parent.dev_tool_window.geometry("{}x{}".format(width, height))
    parent.dev_tool_window.protocol("WM_DELETE_WINDOW", close_window)
